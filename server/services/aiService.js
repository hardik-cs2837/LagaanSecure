...
class AiService {
  ...
  async callExternalLLM(query, userContext, language) {
    const systemPrompt = `You are "Kisaan Copilot", an AI agricultural market advisor for Indian farmers and buyers on KisaanConnect.
      ...
      if (q.includes('प्याज') || q.includes('onion')) {
        return {
          reply: language === 'hi' 
            ...
            : '🧅 Today\'s modal rate for Onion in Nashik/Lasalgaon APMC is ₹1,140 - ₹1,450/quintal. Direct buyers on KisaanConnect are procuring Grade A at ₹1,400+/qtl with zero intermediary commission.',
          engine: 'KisaanConnect Agricultural Expert Engine (Deterministic Fallback)',
          isRealAi: false,
      ...
      if (q.includes('गेहूं') || q.includes('wheat')) {
        return {
          reply: language === 'hi'
            ...
            : '🌾 Lokwan/Sharbati Wheat is currently benchmarked at ₹2,250 - ₹2,550/quintal. Institutional flour mills on KisaanConnect are offering ~₹2,500/qtl for Grade A clean lot.',
          engine: 'KisaanConnect Agricultural Expert Engine (Deterministic Fallback)',
          isRealAi: false,
    ...
    if (q.includes('बिचौल') || q.includes('middleman') || q.includes('markup') || q.includes('save') || q.includes('कमा') || q.includes('earn') || q.includes('commission')) {
      return {
        reply: language === 'hi'
          ...
          : '💰 In traditional supply chains, intermediary margins (commission agent 6%, primary wholesaler 12%, retail 20%) take up to 40% of the consumer rupee. Selling direct on KisaanConnect yields an estimated +₹300 to ₹500/quintal higher net realization for the farmer.',
        engine: 'KisaanConnect Agricultural Expert Engine (Deterministic Fallback)',
        isRealAi: false,
